...
\begin{enumerate}
\item
Reduction on proofs and paths does \emph{not} respect substitution.  For example, let $F \equiv \lambda x:\Omega.x$.  Then we have
\begin{align}
\reff{\lambda y : \Omega. z}_{\bot \bot} \reff{\bot} & \rightarrow z \{ y:= \reff{\bot} : \bot = \bot \} \equiv \reff{z} \nonumber \\
(\reff{\lambda y : \Omega.z}_{\bot \bot} \reff{\bot}) [z := F] & \equiv \reff{\lambda x : \Omega.F}_{\bot \bot} \reff{\bot}
\label{eq:exp1} \\
\label{eq:exp2}
\reff{z}[z:=F] & \equiv \reff{F} \equiv \reff{\lambda x : \Omega.x}
\end{align}
Expression (\ref{eq:exp1}) does not reduce to (\ref{eq:exp2}).  Instead, (\ref{eq:exp1}) reduces to
$$ F \{ y := \reff{\bot} : \bot = \bot \} \equiv \triplelambda e : x =_\Omega x'. e \enspace . $$
\item
