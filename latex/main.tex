...
Using propositional extensionality, we can construct a path of type $\top = \top \rightarrow \top$, and hence a proof of $\top \rightarrow (\top \rightarrow \top)$.
...
Then we have
\begin{align*}
\Gamma & \vdash \lambda p:\top \rightarrow x. e^+ (p I) & : & (\top \rightarrow x) \rightarrow y \\
\Gamma & \vdash \lambda m : y. \lambda n : \top. e^- m & : & y \rightarrow (\top \rightarrow x) \\
\Gamma & \vdash \univ*{\lambda p:\top \rightarrow x. e^- m}{\lambda m:y. \lambda n:\top. e^- m} & : & (\top \rightarrow x) =_\Omega y
\end{align*}
